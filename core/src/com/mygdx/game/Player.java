...
public class Player {
    ...
    public Player(TextureRegion img, TextureRegion img2, TextureRegion img3, World world) {
        ...
        sprite.setScale(2);
        position = new Vector2(Gdx.graphics.getWidth() / 2 - 300,
            sprite.getScaleY() * sprite.getHeight() / 2 + 500);
        this.world = world;
        ...
        this.Rectangle = new Rectangle(sprite.getX(), sprite.getY(), sprite.getWidth(),
            sprite.getHeight());
    }
    ...
    public void Update(float deltaTime) {
        ...
        for (Body b : bodies) {
            ...
            if (!Intersector.overlaps(temp.getRect(), Rectangle)) {
                ...
                if (position.x - (((InteractivetileObject) b.getFixtureList().get(0)
                    .getUserData()).bounds.x) <= 0) {
                    position.x -= 5;
                }
                if (position.y - (((InteractivetileObject) b.getFixtureList().get(0)
                    .getUserData()).bounds.y) <= 0) {
                    position.y -= 5;
                }
                if (position.x + (((InteractivetileObject) b.getFixtureList().get(0)
                    .getUserData()).bounds.x) >= 0) {
